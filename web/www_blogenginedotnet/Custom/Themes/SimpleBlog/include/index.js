...
//custom...
//get the elements too look at and wire up evt listeners
//this is needed for the front page, so the custom map container resizes appropriately
var _mapct = document.getElementById('mapct');
var _banner = document.getElementById('banner-header');
var _onwinresize = function(){
	//map container if any
	if(_mapct){
		_mapct.style.top = document.getElementsByTagName('header')[0].offsetHeight + 'px';
		_mapct.style.bottom = document.getElementsByTagName('footer')[0].offsetHeight + 'px';
	}
	//banner
	if(_banner){
		if(_banner.offsetWidth > 1920){
			_banner.style.backgroundSize= '100% 100%';
		}
		else {
			_banner.style.backgroundSize = null;
		}
	}
}
setTimeout(_onwinresize, 250); //so ie does not mess the stuff
